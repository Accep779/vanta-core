...
// Load configuration
const config = JSON.parse(fs.readFileSync('/home/nodevs/.openclaw/credentials/neon.json', 'utf8'));
...
// List engagements
app.get('/api/engagements', authenticate, async (req, res) => {
  try {
...
app.get('/api/engagements/:id', authenticate, async (req, res) => {
  try {
    const { id } = req.params;
...
// Create engagement
app.post('/api/engagements', authenticate, async (req, res) => {
  try {
    const {
      target_domain,
      target_name,
...
// Start engagement
app.post('/api/engagements/:id/start', authenticate, async (req, res) => {
  try {
    const { id } = req.params;
...
// Pause engagement
app.post('/api/engagements/:id/pause', authenticate, async (req, res) => {
  try {
    const { id } = req.params;
...
app.post('/api/engagements/:id/approve', authenticate, async (req, res) => {
  try {
    const { id } = req.params;
...
// Get finding by ID
app.get('/api/findings/:id', authenticate, async (req, res) => {
  try {
    const { id } = req.params;
...
// Update finding
app.patch('/api/findings/:id', authenticate, async (req, res) => {
  try {
    const { id } = req.params;
